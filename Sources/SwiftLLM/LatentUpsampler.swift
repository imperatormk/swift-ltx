...
public class UpsamplerWeightLoader {
    ...
    func loadInitial() -> (conv: Conv3DWeightF32, normW: Tensor, normB: Tensor) {
        let conv = loadConv3d("initial_conv", C_in: 128, C_out: midChannels)
        let nw = loadTensorF32("initial_norm.weight")
        let nb = loadTensorF32("initial_norm.bias")
        return (conv, nw, nb)
    }

    /// Load a single resblock.
    func loadPreResBlock(_ i: Int) -> UpsamplerResBlockWeights {
        loadResBlock("res_blocks.\(i)", C: midChannels)
    }

    /// Load upsample conv (512→2048).
    func loadUpsampleConv() -> Conv3DWeightF32 {
        loadConv2dAsConv3d("upsampler.0", C_in: midChannels, C_out: midChannels * 4)
    }

    /// Load a single post-resblock.
    func loadPostResBlock(_ i: Int) -> UpsamplerResBlockWeights {
        loadResBlock("post_upsample_res_blocks.\(i)", C: midChannels)
    }

    /// Load final conv (512→128).
    func loadFinalConv() -> Conv3DWeightF32 {
        loadConv3d("final_conv", C_in: midChannels, C_out: 128)
    }
}
...
public func upsamplerForward(_ latent: MTLBuffer, loader: UpsamplerWeightLoader,
    ...
    var x: MTLBuffer = autoreleasepool {
        let initial = loader.loadInitial()
    ...
    for i in 0..<4 {
        x = autoreleasepool {
            let rb = loader.loadPreResBlock(i)
    ...
    x = autoreleasepool {
        ...
        let upConv = loader.loadUpsampleConv()
        var up = conv3dF32(folded, weight: upConv.weight, bias: upConv.bias,
        ...
        let out = transposeHSF32(up, seqLen: C, nHeads: F, headDim: newH * newW)
    ...
    for i in 0..<4 {
        x = autoreleasepool {
            let rb = loader.loadPostResBlock(i)
    ...
    x = autoreleasepool {
        t0 = CFAbsoluteTimeGetCurrent()
        let finalConv = loader.loadFinalConv()
